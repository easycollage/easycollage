...
import { CalendarDays, Download, LayoutGrid, MessageCircle, Table2, ListFilter } from "lucide-react";
...
const CALCOM_URL = process.env.NEXT_PUBLIC_CALCOM_URL || "https://cal.com/easycollege";
...
export function RankFinderClient() {
  ...
  const selectedBranchText = filters.branches ? filters.branches.split(",").join(", ") : "all preferred branches";
  const whatsappMessage = encodeURIComponent(
    `Hi EasyCollege, I am confused about web options. My EAMCET rank is ${filters.rank || "not entered"} and I am checking ${selectedBranchText}. Please guide me.`
  );
  const whatsappUrl = WHATSAPP_NUMBER
    ? `https://wa.me/${WHATSAPP_NUMBER}?text=${whatsappMessage}`
    : `https://wa.me/?text=${whatsappMessage}`;

  function handleLeadUnlock(data: { name: string; phone: string; rank: string; category?: string; gender?: string }) {
    sessionStorage.setItem(STORAGE_KEY, "1");
    setLeadSubmitted(true);
    // Pre-fill rank, category, gender from what they entered in the form
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6">
        ...
        <div className="flex gap-6">
          ...
          <div className="flex-1 min-w-0">
            {isWebOptionsMode && (
              <div className="mb-4 border border-green-100 bg-green-50 rounded-xl p-4 flex flex-col md:flex-row md:items-center md:justify-between gap-3">
                <div>
                  <h2 className="text-sm font-semibold text-gray-900">
                    Confused about web options?
                  </h2>
                  <p className="text-xs text-gray-600 mt-0.5">
                    Book a quick counselling call with us or send your rank on WhatsApp for personal guidance.
                  </p>
                </div>
                <div className="flex flex-col sm:flex-row gap-2">
                  <a
                    ...
                    target="_blank"
                    rel="noreferrer"
                    className="inline-flex items-center justify-center gap-2 rounded-lg bg-gray-900 px-3 py-2 text-xs font-semibold text-white hover:bg-gray-800"
                  >
