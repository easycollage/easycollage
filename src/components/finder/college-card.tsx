    ...
    <div className="group border border-gray-100 rounded-xl p-5 hover:border-green-200 hover:shadow-md transition-all duration-200 bg-white">
      ...
      <div className="flex items-center justify-between pt-3 border-t border-gray-50">
        <div className="flex items-center gap-1.5 text-xs text-gray-600">
          ...
          <span>
            Rank: <span className="font-semibold text-gray-900 font-mono">{formatRank(cutoffRankStart)}</span>
            {" – "}
            <span className="font-semibold text-gray-900 font-mono">{formatRank(cutoffRankEnd)}</span>
          </span>
        </div>
        <div className="flex items-center gap-0.5 text-xs text-gray-500">
          <IndianRupee className="w-3 h-3" />
          <span>{formatFee(college.tuitionFee)}/yr</span>
        </div>
      </div>
      ...
      {userRank && (
        <div className="mt-2 text-xs text-green-600 font-medium flex items-center gap-1">
          ...
          Your rank {formatRank(userRank)} is eligible
        </div>
      )}
    </div>
  );
